...
\newcommand{\UNSTABLE}{\textsc{unstable}}
...
\newcommand{\C}[1]{\unskip\qquad\small{\textit{// #1}}}
...
Masstree can arrange for these updates to be ``published'' atomically,
as enabled by leaf permutations, but this is not always possible. Thus,
...
We say two times \(t\) and \(t'\) are \emph{RCU-overlapping} if a
single reader could be active over the entire interval between \(t\)
and \(t'\).
...
\begin{figure}[H]
\begin{programtabbing}
\textbf{find\_leaf}(node \V{root}, key $k$): \\
retry:
...
\> if $n_c = \NIL$: \C{caused by concurrent remove} \\
\>\> goto retry \\
...
\>\> if $v.\V{vsplit} \neq \V{oldv}.\V{vsplit}$: \C{\(n\) split, retry from root} \\
\>\>\> goto retry \\
\> else: \\
\>\> $n \gets n_c$;~ $v \gets v_c$ \\
\> goto descend
\end{programtabbing}
\caption{Find the border node containing a key.}
\label{fig:nlookup}
\end{figure}
...
\Returns{\(\langle \N{int \V{index}}, \N{int \V{position}} \rangle\)}: \\
\> // In practice we implement this with binary search \\
\> $i \gets 0$;~ $\V{klength} = \min(k.\V{length}, 9)$ \\
\> while $i < \V{perm}.\V{size}$: \\
...
\programtabskip
%
\textbf{forward\_to\_key}(leaf $n$, key $k$) \Returns{\(\langle
  \N{leaf, version}\rangle\)}: \\
...
\begin{figure}[H]
\begin{programtabbing}
\textbf{lock\_leaf}(node \V{toproot}, key \(k\)): \\
retryroot:~
   $\V{root} \gets \V{toproot}$;~ \(k \gets \N{reset\_key}(k)\) \\
retry:~ \> $\langle n, v \rangle \gets \text{find\_leaf}(\V{root}, k)$ \\
\> if $k$ has suffix: \C{optimization} \\
\>\> \(x \gets \text{check\_next\_layer}(n, v, k)\) \\
\>\> if \(x \neq \NONE\): \\
...
\programtabskip
%
\textbf{check\_next\_layer}(node \(n\), version \(v\), key \(k\)): \\
...
retry:
\> $\langle n, v \rangle \gets \text{find\_leaf}(\V{root}, k)$ \\
forward:~
if $v.\V{deleted}$: \\
\>\> goto retry \\
...
\>\> \(t \gets n.\V{type}[\pi]\) \\
\>\> \fence \\
\>\> \(\V{suffix} \gets n.\V{ksuffix}[\pi]\);~ \(\V{value} \gets n.\V{value}[\pi]\) \\
\> \fence \\
\> if \(v \oplus \readnow{n.\V{version}} > \V{locked}\): \\
\>\> $\langle n, v \rangle \gets \text{forward\_to\_key}(n, k)$ \\
\>\> goto forward \\
\> else if \(\pi \neq \NONE\) and \(t = \LAYER\): \\
\>\> \(root \gets \V{value}\);~ \(k \gets \N{shift\_key}(k)\) \\
\>\> goto retry \\
\> else if \(\pi \neq \NONE\) and \(t = \UNSTABLE\): \\
\>\> goto forward \\
...
\> \(\langle n, i, \pi, k \rangle \gets \N{lock\_leaf}(\V{toproot}, k)\) \\
retry:~
...
\>\> goto retry \\
insert:~
\> if \(n.\V{size} + n.\V{nremoved} < \LEAFWIDTH\): \\
...
\>\> unlock(\(n\));~ return \\
\> if \(n.\V{nremoved} > 0\): \\
\>\> \(\N{clean}(n)\);~ goto insert \\
\> \(\N{split}(n, i, k, \V{value})\) \\
\programtabskip
%
...
\N{lock\_leaf} to locate and lock the leaf containing the item, then pop
...
However, there is a wrinkle. It is OK to leave the item's state in the
leaf, in a position unreferenced by the permutation, \emph{only} as long
as that state \emph{remains self-consistent}: corresponding to the old
value. Thus, before the slot can be reused, Masstree must dirty the
node. We keep track of this through permutation manipulations and a
per-node \(\V{nremoved}\) counter. The \N{put} function detects when a
removed slot is reused and dirties the node by calling \N{clean}.
...
locks. The lock is a bit stolen from node \V{next} pointers. The mark
part of \(n.\V{next}\) protects the setting of \(n.\V{next}\), and the
...
Masstree detects such trivial nodes and deletes them in the \N{collapse}
...
\noindent%
Note that the \N{collapse} function will never try to delete the root
node. A trivial root requires more work to fix, as we'll see.

\subsection{Patching holes}
...
special handling. To see why, check out what would happen if we executed
the usual removal procedure on the parent interior node:
...
Although such mistakes could potentially be detected at \N{put} time
using \N{lowkey} comparisons, extra comparisons are unnecessary in the
common case. We prefer to avoid the problem
in a different way, by inducing retry until the tree is fixed. When
a subtree's leftmost leaf is deleted, Masstree detects the issue and
switches to a special \N{patch} procedure. This procedure first sets
...
Now consider what happens when the \ITEM{mno} leaf is deleted. This
makes the entire right-hand subtree redundant.
...
called \N{gclayer}. Logically \N{gclayer} is simple: it locks the
...
\noindent%
and then cleans up the lower layer as appropriate. It locks the root
...
\noindent%
and then marks the child as the new root, switching the higher layer's
value pointer to it.
...
When Masstree encounters a deleted node, it tries again from the current
layer's root. But here, \emph{the root itself} was deleted. We therefore
hook up the \emph{old} root to look to concurrent readers like a child of the
\emph{new} root. This will help concurrent readers find the right children,
even if the root disappeared after concurrent splits.
...
\> else: \\
\>\> \(\N{patch}(p, k, \V{toproot}, \V{layerkey})\) \\
\>\> return \\
...
\begin{figure}[H]
\begin{programtabbing}
\textbf{patch}(interiornode \(n\), key \V{k}, node \V{toproot}, key \V{layerkey}): \\
...
\> \rcufence \\
\> \(\langle n, i, \pi, \N{\_} \rangle \gets \N{lock\_leaf}(\V{toproot}, \V{layerkey})\) \\
...
%
To guard against inappropriate reuse, the \N{insert} procedure's code
