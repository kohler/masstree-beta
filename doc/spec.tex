...
Get has two phases: first, it descends to the leaf responsible for a key;
then, it finds the key in that leaf.
...
Version $v$ of leaf $n$ is guaranteed to be responsible for $k$.
...
However, there is a wrinkle. We must ensure that the node is dirtied
before the slot is reused, or a concurrent reader might see an
inconsistent state. We keep track of this through permutation
manipulations and a per-node \(\V{nremoved}\) counter. The \N{put}
function detects when a removed slot is reused and dirties the node by
calling \N{clean}.
